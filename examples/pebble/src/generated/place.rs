...
pub fn resolve_place<'a, H: Handlers>(
    ...
    match node {
        Expr::Atom(a) => resolve_place_atom(host, a, cx),
        Expr::Prefix { span, .. }
        | Expr::Postfix { span, .. }
...
fn resolve_place_atom<'a, H: Handlers>(
    host: &mut H,
    node: &'a Atom,
    cx: &mut Ctx,
) -> Result<Place<'a, H::Out>> {
    match node {
        Atom::Name(n) => {
            let span = n.span;
            let name = &n.name;
            Ok(Place::AtomName { span, name, _out: ::core::marker::PhantomData })
        }
        Atom::Expr(e) => resolve_place(host, e, cx),
        _ => Err(Error::runtime(
