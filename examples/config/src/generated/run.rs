    ...
    H: dispatch::Handlers,
{
    ...
    let mut pairs = match crate::ConfigParser::parse(crate::Rule::document, &text) {
        Ok(p) => p,
        // `expect` declarations turn pest's rule names into a sentence.
        Err(e) => return Err(vec![diagnostics::render_parse_error(&e, file)]),
    };
    let node = pairs.next().expect("`document` yields exactly one pair");
    ...
    let outcome = ast::build_document(node, file)
        .and_then(|tree| dispatch::eval_document(host, &tree, cx));
    cx.leave();
